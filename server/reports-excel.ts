...
async function buildAccommodationSheet(wb: ExcelJS.Workbook, storage: IStorage, hotelName: string, from?: string, to?: string) {
  ...
  const cols = ["Booking ID", "Guest", "Email", "Phone", "Room", "Room type", "Check-in", "Check-out", "Nights", "Rate/night (KES)", "Total (KES)", "Paid (KES)", "Balance (KES)", "Status", "Notes"];
  addTitleBlock(ws, hotelName, "Accommodation Bookings", from, to, "O");
  ...
  filtered.forEach((b) => {
    ...
    const row = ws.addRow([
      ...
      b.checkIn, b.checkOut, nights, b.rate, b.totalAmount, b.amountPaid, balance, titleCase(b.status), b.notes ?? "",
    ]);
  ...
  autosizeColumns(ws, [10, 18, 22, 14, 10, 10, 11, 11, 8, 14, 12, 12, 12, 12, 20]);
}
...
async function buildFacilitiesSheet(wb: ExcelJS.Workbook, storage: IStorage, hotelName: string, from?: string, to?: string) {
  ...
  filtered.forEach((b) => {
    ...
    const row = ws.addRow([
      ...
      b.startTime ?? "—", b.endTime ?? "—", b.rate, b.totalAmount, b.amountPaid, balance, titleCase(b.status), b.notes ?? "",
    ]);
...
async function buildBarRestaurantSheets(wb: ExcelJS.Workbook, storage: IStorage, hotelName: string, from?: string, to?: string) {
  ...
  const cols = ["Order ID", "Outlet", "Reference", "Customer", "Email", "Date", "Payment method", "Status", "Total (KES)"];
  addTitleBlock(ordersWs, hotelName, "Bar & Restaurant Orders", from, to, "I");
  ...
  filtered.forEach((o) => {
    ...
    const row = ordersWs.addRow([
      ...
      o.orderDate, o.paymentMethod ? titleCase(o.paymentMethod) : "—", titleCase(o.status), o.totalAmount,
    ]);
    row.getCell(9).numFmt = KES_FMT;
  ...
  if (filtered.length === 0) {
    ...
    totalsRow.getCell(9).numFmt = KES_FMT;
    totalsRow.getCell(9).alignment = { horizontal: "right" };
    styleTotalsRow(totalsRow);
    ordersWs.addRow(["", "", "", "", "", "", "", "Bar (paid)", barTotal]).getCell(9).numFmt = KES_FMT;
    ordersWs.addRow(["", "", "", "", "", "", "", "Restaurant (paid)", restaurantTotal]).getCell(9).numFmt = KES_FMT;
  }

  ordersWs.autoFilter = { from: { row: 4, column: 1 }, to: { row: 4, column: cols.length } };
  ordersWs.views = [{ state: "frozen", ySplit: 4 }];
  autosizeColumns(ordersWs, [8, 12, 14, 18, 22, 11, 14, 10, 12]);
